...
public class ImgsFillDynamic : MonoBehaviour
{
    ...
    //public Color[] PercentByColor; // 더 이상 사용 X

    //public bool IsUsingMaxColor = false; // 더 이상 사용 X
    //public Color MaxColor; // 더 이상 사용 X
    ...
    void SetImageColor()
    {
        Color redColor = Color.red; // 빨간색으로 고정
        for (int i = 0; i < this.ImgFacterTarget.Length; i++)
            this.ImgFacterTarget[i].color = redColor;
    }
}
